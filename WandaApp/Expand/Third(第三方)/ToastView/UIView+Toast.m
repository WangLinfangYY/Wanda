...
static const NSTimeInterval CSToastShortDuration    = 1.5;
...
static const NSString * CSMessageToastTimerKey  = @"CSMessageToastTimerKey";
...
-(NSString*)getShowMessage:(NSString*)message {
//    if ([QHCommonUtil strNilOrEmpty:message]) {
//        return @"系统内部错误";
//    } else {
        return message;
//    }
...
- (void)makeToast:(NSString *)message {
    ...
    [self makeToast:[self getShowMessage:message] duration:CSToastShortDuration position:CSToastPositionCenter];
}

- (void)makeToast:(NSString *)message position:(id)position {
    [self makeToast:[self getShowMessage:message] duration:CSToastShortDuration position:position];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
    UIView *toast = [self viewForMessage:[self getShowMessage:message] title:nil image:nil];
    [self showToast:toast duration:duration position:position];  
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title {
    UIView *toast = [self viewForMessage:[self getShowMessage:message] title:title image:nil];
    [self showToast:toast duration:duration position:position];  
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position image:(UIImage *)image {
    UIView *toast = [self viewForMessage:[self getShowMessage:message] title:nil image:image];
    [self showToast:toast duration:duration position:position];  
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title image:(UIImage *)image {
    UIView *toast = [self viewForMessage:[self getShowMessage:message] title:title image:image];
    [self showToast:toast duration:duration position:position];  
}
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 4 -  CSToastVerticalPadding);
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (message != nil) {
        ...
        messageLabel.font = [UIFont systemFontOfSize:CSToastFontSize];//   [UIFont fontWithName:kFontName size:CSToastFontSize];
...
#pragma mark - websocket 下拉弹框消息显示
- (void)showMessageToast:(NSString*)message
{
    [self showMessageToast:message duration:5.0f];
}

- (void)showMessageToast:(NSString*)message duration:(NSTimeInterval)duration {
    MessageToastView* messageView = [[MessageToastView alloc] initWithMessage:message];
    messageView.translatesAutoresizingMaskIntoConstraints = YES;
    messageView.clipsToBounds = YES;
    UITapGestureRecognizer *recognizer = [[UITapGestureRecognizer alloc] initWithTarget:messageView action:@selector(handleMessageToastTapped:)];
    ...
    int height = messageView.frame.size.height;
    messageView.frame = CGRectMake(0, navBarOrigin, kScreenWidth, 0);
    
    [UIView animateWithDuration:1.0f
                          delay:0.0
                        options:UIViewAnimationOptionAllowUserInteraction
                     animations:^{
                         messageView.frame = CGRectMake(0, navBarOrigin, kScreenWidth, height);
                     } completion:^(BOOL finished) {
                         NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:duration target:self selector:@selector(messageToastShowFinished:) userInfo:messageView repeats:NO];
                         // associate the timer with the toast view
                         objc_setAssociatedObject (messageView, &CSMessageToastTimerKey, timer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                     }];
}

- (void)messageToastShowFinished:(NSTimer *)timer {
    [self hideMessageToastView:[timer userInfo]];
}

- (void)handleMessageToastTapped:(UITapGestureRecognizer *)recognizer {
    NSTimer *timer = (NSTimer *)objc_getAssociatedObject(self, &CSMessageToastTimerKey);
    [timer invalidate];
    [self hideMessageToastView:recognizer.view];
}

- (void)hideMessageToastView:(UIView*)messageView {
    [UIView animateWithDuration:1.0f
                          delay:0.0
